...
while CONT                          % while energy balance does not close
    ...
    biochem_in.TDP          = leafbio.TDP;  % DD Added Jan 2018 for Passing Temperature Dependence Parameters for PS parameters
    ...
    if options.Fluorescence_model==2    % specific for the v.Caemmerer-Magnani model
        ...
        b                   = @biochemical_new; % specific for Berry-v.d.Tol model
    ...
    Kdh                 = biochem_out.Kd;
    Kfh                 = biochem_out.Kf;
    phPSIIh             = Kfh./(Kfh+Kph+Kdh+Knh); % PSII Yeild Shaded
    phPSIIh             = biochem_out.ps; % photochemical yeild shade
    %
    ...
    Kdu                 = biochem_out.Kd;
    Kfu                 = biochem_out.Kf;
    phPSIIu             = Kfu./(Kfu+Kpu+Kdu+Knu); % PSII Yeild Sunlit
    phPSIIu             = biochem_out.qQ; % photochemical yeild sunlit
    %
    ...
    [lEch,Hch,ech,Cch,VPDh]     = heatfluxes((LAI+1)*(raa+rawc),rcwh,Tch,ea,Ta,e_to_q,0,Ca,Cih);
    [lEcu,Hcu,ecu,Ccu,VPDu]     = heatfluxes((LAI+1)*(raa+rawc),rcwu,Tcu,ea,Ta,e_to_q,0,Ca,Ciu);
...
if options.calc_vert_profiles   
    ...
    % DD Added 10.10
    [phPSIIuL   ]           = meanleaf(canopy,phPSIIu,          'angles');   % [nli,nlo,nl]      mean phiPSII sunlit leaves
    [VPDu1d  ]           = meanleaf(canopy,VPDu,          'angles');   % [nli,nlo,nl]      mean VPD sunlit leaves
    ...
    % DD added 10.10.2017
    profiles.phPSII         = ((1-Ps(1:nl)).*phPSIIh      + Ps(1:nl).*(phPSIIuL));         %[nl]   mean PSII eff leaves, per layer
    profiles.VPDh        = VPDh;                                                 % [nl] shaded leaves VPD profiles
    profiles.VPDu1d      = VPDu1d;                                               % [nl] sunlit leaves VPD profiles
    profiles.VPD1d       = (1-Ps(1:nl)).*VPDh       + Ps(1:nl).*(VPDu1d);         % [nl] mean VPD leaves, per layer
...
% Code added by debsunder dutta
% %%%%%%%%%%
can_cond = LAI*(Fc*(1./rcwh) + meanleaf(canopy,(1./rcwu),'angles_and_layers',Ps)); % net canopy conductance
can_cond_gs = LAI*(Fc*(gsh) + meanleaf(canopy,(gsu),'angles_and_layers',Ps)); % net Canopy conductance
Agtot           = LAI*(Fc*Agh   + meanleaf(canopy,Agu  ,'angles_and_layers',Ps)); %Gross assimilation GPP
%VPDn          = LAI*(Fc*VPDh   + meanleaf(canopy,VPDu  ,'angles_and_layers',Ps)) %Gross assimilation GPP
% %%%%%%%%%%
...
% Added DD
fluxes.can_cond = can_cond*1000;
fluxes.can_cond_gs = can_cond_gs;
fluxes.Agtot = Agtot;
% %%
