...
%Output_dir      = sprintf(['../output/',Outdir_Name,'/']);
...
if options.calc_vert_profiles
    ...
    if options.calc_fluor
        ...
        % DD added 10.10.2017
        fidphPSII = fopen([Output_dir,'layer_phPSII.dat'],'w');
        %
    end
...
if options.save_headers
    fprintf(fidf,'timestep counter year t Rntot lEtot Htot Rnctot lEctot Hctot Actot Rnstot lEstot Hstot Gtot Resp aPAR aPAR_Cab faPAR aPAR_energyunits iPAR can_cond can_cond_gs Agtot');
    if options.calc_fluor
        fprintf(fidf,' fluortot fluor_yield');
    end
    fprintf(fidf,'\r');
    fprintf(fidf,'""        ""      ""  JulianDay  Wm-2   Wm-2 Wm-2 Wm-2 Wm-2   Wm-2 umolm-2s-1 Wm-2 Wm-2 Wm-2 Wm-2 umolm-2s-1 umolm-2s-1 umolumol-1 Wm-2 umolm-2s-1 mms-1 umolm-2s-1 umolm-2s-1');
    if options.calc_fluor
   ...
   % fprintf(fidref2, 'fraction of radiation in observation direction *pi / irradiance \r');
    ...
    if options.calc_vert_profiles
        ...
        if options.calc_fluor
            ...
            fprintf(fidNPQ, '\r');
            % DD added 10.10.2017
            fprintf(fidphPSII, 'PSII yeild, per layer \r');
            fprintf(fidphPSII, '\r');
            %
        end
    end
